...
- **`~/.gitconfig`** - Git設定（ホストと同期）
- `~/.ssh/` - SSH鍵（ホストと同期、オプション）

> **注記**: セットアップスクリプトは、`~/.gitconfig`と`~/.git-credentials`が存在しない場合、空のファイルを自動的に作成します。これにより、ボリュームマウント時にDockerがディレクトリとして作成するのを防ぎます。Gitの設定は後で以下のように行えます：
> ```bash
...
2. **セットアップまたは環境切り替えを実行** - 証明書は自動的に検出されます:
   ```bash
   bash setup-docker.sh
   # または
   bash switch-env.sh
...
| `~/.git-credentials` | `~/.git-credentials` | HTTPS認証用のGit認証情報 |
...
- **個人設定**: `~/.gitconfig`、`~/.ssh`、`~/.git-credentials` が開発の利便性のためホストと同期されます。`~/.aws` はボリュームマウントで永続化されます
- **機密情報**: 生成された `.env`、`.envs/*.env` にはユーザー情報（UID/GID/Docker GID）が含まれます

> **注意**: `~/.ssh`ディレクトリ全体と`.git-credentials`ファイルがコンテナ内からアクセス可能です。コンテナ内のプロセスはこれらのファイルを読み取りおよび変更できます。信頼できないコードを実行する場合は注意してください。
