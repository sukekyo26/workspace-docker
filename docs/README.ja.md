...
> **補足:** 初回実行時（または `workspace.toml` が存在しない場合）は対話式セットアップが自動的に開始されます。
> 対話式セットアップを再実行するには `bash setup-docker.sh --init` を使用してください。
> デフォルト値で非対話式セットアップするには `bash setup-docker.sh --init --yes` を使用してください。
...
利用可能なプラグインの一覧は[リファレンス — プリインストール済みアプリケーション](reference.ja.md)を参照してください。

> **TOMLバリデーション:** [Even Better TOML](https://marketplace.visualstudio.com/items?itemName=tamasfe.even-better-toml) VS Code拡張機能をインストールすると、同梱の `.taplo.toml` 設定により `workspace.toml` のIDE補完・バリデーションが有効になります。
