...
- **品質保証**: 35のテストスイートとGitHub Actions CI/CD（ShellCheck、Hadolint、スナップショットテスト）
...
利用可能なプラグイン: `proto`, `aws-cli`, `aws-sam-cli`, `claude-code`, `copilot-cli`, `custom-ps1`, `docker-cli`, `github-cli`, `go`, `lazygit`, `rust`, `starship`, `uv`, `zig`（`plugins/*.toml`で定義）
