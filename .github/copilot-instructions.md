...
1. `gh run list --limit 1` で最新 CI 結果を確認する（失敗時はユーザーへ報告・修正を優先）
2. `agent/lessons.md` を **read_file で全行通読**する（上限超過なら古いエントリを削除）
3. エントリが10件以上なら卒業判断を実施する（後述の運用ルール参照）
4. 複雑なタスクは `manage_todo_list` で計画を立ててから着手する
   - **末尾に「卒業判断」タスクを必ず含める**（lessons.md が10件以上の場合）
...
| 卒業判断（10件到達時） | 類似統合 →「カスタマイズファイルの選択基準」表に従い適切なカスタマイズファイルへの昇格を提案 |

### 卒業プロセス（10件到達時）

1. 類似エントリを統合して件数を減らす
2. 残存エントリについて「カスタマイズファイルの選択基準」表で昇格先候補を決める
3. **対応スキルを read_file で読み込む**（卒業文脈ではスキルは自動発動しない）
4. スキルの作成要件を踏まえて「なぜ必要か」と「どのファイルを変更すべきか」をユーザーに提案する（勝手に変更しない）
5. 承認後、スキルの手順に従って作成する
...
### 卒業との関係
- **卒業**: lessons.md 10件到達時に、エントリを上記の適切なファイルへ昇格する作業
- **自己改善**: 作業中にカスタマイズ体系の問題（古い記載・不足・矛盾）に気づいた時の改善提案
- 両者は独立したトリガーで発動する。卒業は自己改善の一形態だが、自己改善は卒業に限らない
